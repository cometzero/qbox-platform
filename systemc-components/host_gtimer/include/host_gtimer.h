...
class host_gtimer : public sc_core::sc_module
{
    ...
    uint32_t read32(uint32_t offset, int64_t absolute_ns) const;
    void write32(uint32_t offset, uint32_t value, int64_t absolute_ns);
...
public:
    ...
    host_gtimer(sc_core::sc_module_name name, gs::arm_system_counter& counter);
    host_gtimer(sc_core::sc_module_name name, sc_core::sc_object* counter);

    void before_end_of_elaboration() override;
    FrontendSnapshot snapshot_at(int64_t absolute_ns) const;
